#if defined(A12_2A)
!
! REAL FUNCTION recip_tau_rad_showman
...
REAL FUNCTION recip_tau_rad_showman(temperature, pressure_pa, planet)
  ...
  implicit None
  ! Purpose:
  !          Provides temperature relaxation Timescale for
  ! planets HD209458b and HD189733b from Showman et al (2008)
  !
  ...
  IF (lhook) CALL dr_hook('RECIP_TAU_RAD_SHOWMAN',zhook_in,zhook_handle)
  ...
  if (T_inc == 0 .or. T_inc == +1) then
     ...
     IF (lhook) CALL dr_hook('RECIP_TAU_RAD_SHOWMAN',zhook_out,zhook_handle)
  return
