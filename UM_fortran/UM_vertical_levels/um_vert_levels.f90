program um_vert_levels
  ...
  real :: max_sig, min_sig, sig_sep, top_of_model_m
  ...
  print*, 'This program currently creates a uniform (in sigma) set of vertical levels'




  print*, 'Please enter the maximum and minimum sigma (not 1.0 and 0.0-these are added!!)'
  print*, 'Max'
  read(*,*) max_sig
  print*, 'Min'
  read(*,*) min_sig
  print*, 'Now the number of required Levels (theta i.e. rho +1)'
  read(*,*) n_sig
  ! One space is reserved for Z=0
  sig_sep=(max_sig-min_sig)/(n_sig-2)
  print*, 'Using Even Sigma separations from:', max_sig, ' to:', min_sig
  print*, 'Using ', n_sig,' steps of:', sig_sep
  ...
  ! Set up the sigma levels
  sigma(1)=1.0
  do i=2, n_sig
     sigma(i)=max_sig-(i-2)*sig_sep
  end do
...
!     write(eta_rho_format(i),'(f10.8)') eta_rho(i)
     write(eta_theta_format(i),*) eta_theta(i)
     write(eta_rho_format(i),*) eta_rho(i)
  end do!
!  write(eta_theta_format(n_sig),'(f10.8)') eta_theta(n_sig)
  write(eta_theta_format(n_sig),*) eta_theta(n_sig)
