function find_interp_var_boundaries_3d, var_in, $
                                        which_index, $
                                        grid_index_bounds, $
                                        PERIODIC=PERIODIC, $
                                        VALUE=VALUE,$
  ...
  var_dim=size(var_in)
  err=0
  if (var_dim(0) ne 3) then err=1
  if (which_index lt 0 or which_index gt 2) then err=1
  ...
  if (KEYWORD_SET(polar)) then begin 
     ...
     ; try to acces it
     err_temp=index_per
     if (index_per lt 0 or index_per gt 2) then err=1
  ...
  if (which_index eq 0) then begin
     ...
     var_interp_bounds=safe_alloc_arr(3, arr_size, /float_arr)
     ...
     print, 'find_interp_var_boundaries_3d'
     stop
  endelse
  ...
  if (grid_index_bounds(0) eq -1 or grid_index_bounds(1) eq -1) then begin
     ...
     if KEYWORD_SET(value) then begin
        ...
        if (grid_index_bounds(0) eq -1) then begin
           ...
           print, 'see find_interp_var_boundaries_3d'
           stop
        endif
        ...
        if (index_per eq 0) then nlon=n_elements(var_in(*,0,0))
        if (index_per eq 1) then nlon=n_elements(var_in(0,*,0))
        if (index_per eq 2) then nlon=n_elements(var_in(0,0,*))
        ...
        ; Here we simply set the values to missing
