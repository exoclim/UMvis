function find_interp_var_boundaries_4d, var_in, $
                                        which_index, $
                                        grid_index_bounds, $
                                        PERIODIC=PERIODIC, $
                                        VALUE=VALUE,$
  ...
  var_dim=size(var_in)
  err=0
  if (var_dim(0) ne 4) then err=1
  if (which_index lt 0 or which_index gt 3) then err=1
  ...
  if (KEYWORD_SET(polar)) then begin 
     ...
     ; try to acces it
     err_temp=index_per     
     if (index_per lt 0 or index_per gt 3) then err=1
  ...
  if (which_index eq 0) then begin
     ...
     var_interp_bounds=safe_alloc_arr(4, arr_size, /float_arr)
     ...
     arr_size(3)=2
     var_interp_bounds=safe_alloc_arr(4, arr_size, /float_arr)
     nvar_max=var_dim(4)-1
  endif else begin
     print, 'Error invalid index selected in call to'
     print, 'find_interp_var_boundaries_4d'
     stop
  endelse
  ...
  if (grid_index_bounds(0) eq -1 or grid_index_bounds(1) eq -1) then begin
     ...
     if KEYWORD_SET(value) then begin
        ...
        if (grid_index_bounds(0) eq -1) then begin
           ...
           print, 'see find_interp_var_boundaries_4d'
           stop
        endif
        ...
        if (index_per eq 0) then nlon=n_elements(var_in(*,0,0,0))
        if (index_per eq 1) then nlon=n_elements(var_in(0,*,0,0))
        if (index_per eq 2) then nlon=n_elements(var_in(0,0,*,0))
        if (index_per eq 3) then nlon=n_elements(var_in(0,0,0,*))
        ...
        for ilon=0, nlon-1 do begin
           ...
           endif else if (index_per eq 2) then begin
              if  (which_index eq 0) then begin
                 var_interp_bounds(1,*,ilon,*)=$
                    var_in(grid_index_bounds(0),*,iilon,*)
              endif else if  (which_index eq 1) then begin
           ...
           endif else if (index_per eq 3) then begin
              if  (which_index eq 0) then begin
                 var_interp_bounds(1,*,*,ilon)=$
                    var_in(grid_index_bounds(0),*,*,iilon)
              endif else if  (which_index eq 1) then begin
        ...
        ; Simply set these values as missing
        var_interp_bounds(*,*,*,*)=missing
     endif
     ; Now have we hit a periodic boundary
     ; If we are at the edges,
     ; but we aren't just interpolating between two points!
  endif else if ((grid_index_bounds(0) eq 0 and $
                  grid_index_bounds(1) eq nvar_max and nvar_max ne 1)) $
     or ((grid_index_bounds(0) eq nvar_max and $
          grid_index_bounds(1) eq 0) and nvar_max ne 1) then begin
